...
class inD_RecordingModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        """
        if stage == "test":
            self.test = inD_RecordingDataset(self.data_path, self.recording_id, self.sequence_length, self.features_tracks, self.features_tracksmeta,self.motion_obj, self.transform)
        if stage == "predict":
            self.predict = inD_RecordingDataset(self.data_path, self.recording_id, self.sequence_length, self.features_tracks, self.features_tracksmeta,self.motion_obj, self.transform)
        if stage == "fit":
            full = inD_RecordingDataset(self.data_path, self.recording_id, self.sequence_length, self.features_tracks, self.features_tracksmeta,self.motion_obj, self.transform)
            data_size = len(full)
